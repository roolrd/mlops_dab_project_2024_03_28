...
| prod        | The `prod` deployment target is part of the CD pipeline. Latest rool_test content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-my_mlops_project-model    | /prod-my_mlops_project-experiment    |
...
resources will be deployed to the prod workspace on pushing code to the `rool_test` branch.
...
When you about to cut a release, you can create and merge a PR to merge changes from main to rool_test. Continuous deployment automation will deploy `prod` resources to the prod workspace.
